...
test.describe("Checkout Flow", () => {
	...
	test("unauthenticated user clicking subscribe triggers auth redirect or error", async ({
		...
		// In production mode, should redirect to login or show an error
		// In demo mode, redirects to dashboard with checkout=success
		await expect(
...
test.describe("Checkout Success Sync", () => {
	test("dashboard handles checkout success params gracefully", async ({ page }) => {
		// Visit dashboard with checkout params but unauthenticated
		// In production mode, should redirect to login
		// In demo mode, middleware bypasses auth so dashboard loads
