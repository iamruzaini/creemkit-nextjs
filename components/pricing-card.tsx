...
export function PricingCard({ plan, discountCode }: { plan: PricingPlan; discountCode?: string }) {
  ...
  return (
    <div
      className={`bg-bg-secondary rounded-2xl border-3 p-7 flex flex-col relative group hover:-translate-y-1 transition-all duration-300 ${
        plan.popular ? "border-accent-orange" : "border-border hover:border-accent-orange/30"
      }`}
      style={{
    ...
    >
      {plan.popular && (
        <span className="self-start px-4 py-1.5 bg-accent-orange text-black text-xs font-extrabold uppercase tracking-widest rounded-full mb-4 border-2 border-black animate-pulse">
          🔥 Most Popular
        </span>
      )}
